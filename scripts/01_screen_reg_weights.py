...
save_path = os.path.join(glob_dir, r'Testfit\FCS_regMEM_naive_Amp_weightScreen')
...
reg_weights = [1e-6, 1e-3, 1e-1, 1e0]
...
for i_file, dir_name in enumerate(in_dir_names):
    ...
    for reg_weight in reg_weights:
        ...
        message += f' with reg weight {reg_weight}:' if spectrum_type in ['reg_MEM', 'reg_CONTIN'] else ':'
