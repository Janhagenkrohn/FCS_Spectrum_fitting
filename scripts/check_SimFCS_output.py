...
folder = r'\\samba-pool-schwille-spt.biochem.mpg.de\pool-schwille-spt\P6_FCS_HOassociation\Data\simFCS2_simulations\3a'
file = 'batch3a_cond3_rep01.bin'
sd_method = 'bootstrap'

sim_time_resolution = 1E-6 # 1 us sampling in simulation settings
...
out_table.to_csv(os.path.splitext(path)[0] + '_ACF_ch0.csv', 
                 index = False, 
                 header = False)
...
plt.savefig(os.path.splitext(path)[0] + '_ACF_ch0.png', 
            dpi=300)
plt.close()
...
out_table.to_csv(os.path.splitext(path)[0] + '_PCMH_ch0.csv', 
                 index = False, 
                 header = True)

out_table = pd.DataFrame(data = {'Bin Times [s]': bin_times,
                                 'Mandel Q': Mandel_Q_series})
out_table.to_csv(os.path.splitext(path)[0] + '_Mandel_Q_ch0.csv', 
                 index = False, 
                 header = True)
...
plt.savefig(os.path.splitext(path)[0] + '_PCMH_ch0.png', 
            dpi=300)
plt.close()
